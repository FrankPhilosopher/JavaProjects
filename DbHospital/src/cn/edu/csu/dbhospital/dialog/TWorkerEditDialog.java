...
public class TWorkerEditDialog extends TitleAreaDialog {
	 ...
	 * @param parentShell
	 */
	public TWorkerEditDialog(Shell parentShell, TWorker worker) {
		super(parentShell);
	...
	protected Control createDialogArea(Composite parent) {
		...
		fd_btn_modify.right = new FormAttachment(btn_statics, -6);
	...
	private void initData() {
		if (worker != null) {
			txt_realname.setText(worker.getRealname());
			txt_phone.setText(worker.getPhone());
			txt_name.setText(worker.getName());
			txt_pwd.setText(worker.getPassword());
			if (worker.getGander() == SystemUtil.GANSER_MALE_VALUE) {
				cb_gander.setText(SystemUtil.GANSER_MALE);
			} else {
				cb_gander.setText(SystemUtil.GANSER_FEMALE);
			}
			if (worker.getCanModify() == SystemUtil.RIGHT_YES) {
				btn_modify.setSelection(true);
			}
			if (worker.getCanStatics() == SystemUtil.RIGHT_YES) {
				btn_statics.setSelection(true);
			}
		}
	}
	...
	protected void buttonPressed(int buttonId) {
		if (buttonId == IDialogConstants.OK_ID) {
			...
			if (!worker.getPassword().equalsIgnoreCase(MD5Util.encrypt(txt_pwd.getText()))) {
				worker.setPassword(MD5Util.encrypt(txt_pwd.getText()));
			}
