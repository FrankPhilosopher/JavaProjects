...
<body style="background: none">
	...
	<form name="form1" method="post" action="actionCar.jsp?action=save">
		<div class="table">
			<table class="select-table">
				...
				<tr>
					...
					<td><input name="number" id="number"
						value="<%=((Car) request.getAttribute("car")).getNumber()%>"
						type="text" class="text" /></td>
					<th>司机姓名(*)</th>
					<td class="last"><input name="driver" id="driver"
						value="<%=((Car) request.getAttribute("car")).getDriver()%>"
				...
				<tr>
					...
					<td><input name="weight" id="weight"
						value="<%=((Car) request.getAttribute("car")).getWeight()%>"
						type="text" class="text" /></td>
					<th>承载体积(*)</th>
					<td class="last"><input name="volumn" id="volumn"
						value="<%=((Car) request.getAttribute("car")).getVolumn()%>"
						type="text" class="text" />
					</td>
				</tr>
				<tr>
					<th class="first">车辆状态(*)</th>
					<td><select name="status" id="status" style='width:130px'>
							<%
								int status = ((Car) request.getAttribute("car")).getStatus();
								out.print("<option value='0'");
								if (status == 0) {
									out.print(" selected='selected'");
								}
								out.print(">空闲的</option>");

								out.print("<option value='1'");
								if (status == 1) {
									out.print(" selected='selected'");
								}
								out.print(">在站点</option>");

								out.print("<option value='2'");
								if (status == 2) {
									out.print(" selected='selected'");
								}
								out.print(">在途中</option>");
							%>
					...
					<td><select name="currentDistribution"
						id="currentDistribution" style="width:130px">
							<%
								...
								int currentDistribution = ((Car) request.getAttribute("car"))
										.getCurrentDistribution();
								for (int i = 0; i < distributionList.size(); i++) {
									Distribution distribution = distributionList.get(i);
									if (currentDistribution != distribution.getId()) {
				...
				<tr>
					<th><input type="button" class="button" onclick="checkForm()"
						value="保  存" /> <input type="button" class="button"
						onclick="window.location.href='actionCar.jsp?action=forwardList&pageIndex=1'"
						value="返 回" style="cursor:pointer;" />
					</th>
					<th><label id="message" style="color: red; float: left;">
							<%=request.getAttribute("message")%> </label>
					</th>
					<th><input name="id"
						value="<%=((Car) request.getAttribute("car")).getId()%>"
