...
public class EnterManageAction extends Action implements IWorkbenchAction {
	...
	public void run() {
		...
		if (editor != null) { // 如果编辑器已经打开
			if (EnterEditor.STYLE == EnterEditor.ENTER_ONLY) // 如果当前的编辑器是 入场控制
				workbenchPage.bringToTop(editor); // 则显示该编辑器
			else {
				workbenchPage.closeEditor(editor, true); // 关闭当前编辑器 并重新打开  true 表示save
				EnterEditor.STYLE = EnterEditor.ENTER_ONLY;
				try {
					workbenchPage.openEditor(EnterEditorInput.eei, EnterEditor.ID);
