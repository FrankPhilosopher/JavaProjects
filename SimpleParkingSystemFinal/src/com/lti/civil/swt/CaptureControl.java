...
public class CaptureControl extends AbstractVideoControl<com.lti.civil.Image> {
	...
	public CaptureControl(Composite parent, int style, CaptureSystemFactory factory, int deviceIndex) {
	...
	public boolean saveImage(String path) {
		try {
			getImageControl().exportImageToJpegFile(path);
			return true;
		} catch (IOException e) {
			return false;
		}
	 ...
	 * @param turn
	 */
	public void setImageOverturn(boolean turn) {
		getImageControl().setOverturn(turn);
	}
...
//                SWTOptionPane.QUESTION_MESSAGE, null, choices, choices[0]);
		for (int i = 0; i < list.size(); i++) {
			final CaptureDeviceInfo info = list.get(deviceIndex);
...
//            }
	...
	public static void runShell(Shell shell, CaptureSystemFactory factory, CaptureControlListener listener)
			throws CaptureException {
		shell.setLayout(new FillLayout());
		CaptureControl c = new CaptureControl(shell, 0, factory, 0);
	...
	public static void run(Shell parent, CaptureSystemFactory factory, CaptureControlListener listener)
			throws CaptureException {
		Shell shell = new Shell(parent);
		runShell(shell, factory, listener);
	}

	public static void run(Display parent, CaptureSystemFactory factory, CaptureControlListener listener)
			throws CaptureException {
		Shell shell = new Shell(parent);
		runShell(shell, factory, listener);
	}
