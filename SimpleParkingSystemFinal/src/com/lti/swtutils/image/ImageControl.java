...
public class ImageControl extends Composite {
	...
	public void setOverturn(boolean turn) {
		this.overturn = turn;
	}
...
//				    			drawY = (frameHeight - imageHeight) / 2;
						if (overturn) {
							Transform trans = new Transform(image.getDevice());
							trans.translate(imageWidth / 2, imageHeight / 2);
							trans.rotate(180);
							trans.translate(-imageWidth / 2, -imageHeight / 2);
						...
						if (USE_GC_TO_SCALE && (scaleImageToFit || doubleLines)
								&& (imageWidth != image.getBounds().width || imageHeight != image.getBounds().height)) {
							// TODO: see http://www.eclipse.org/articles/Article-SWT-images/graphics-resources.html
							// for scaling info.
							// GC.drawImage(Image image, int srcX, int srcY, int srcWidth, int srcHeight, int dstX, int
							// dstY, int dstWidth, int dstHeight)
							e.gc.drawImage(image, 0, 0, image.getBounds().width, image.getBounds().height, drawX,
									drawY, imageWidth, imageHeight);
						} else {
//				    			Image nimage = new Image(device,imageWidth,imageHeight);
		...
		imageProxyLoaderThread = new MessageDrivenThread(ThreadGroupMgr.getDefaultThreadGroup(),
				"MessageDrivenThread for ImageControl", new MessageDrivenThreadListener() {
...
//	}
	...
	private Image doubleLines(Image image) {
		...
		return new Image(getShell().getDisplay(), SWTUtils.getImageData(image).scaledTo(image.getBounds().width,
	...
	private Point calcScaledToExact(Image image) {
		...
		} else if (xRatio > yRatio) // if it is proportionally bigger in the x direction, then fill the x dimension, and
									// the y dimension will have extra space.
		{
			scaledTo.x = frameWidth;
