...
function validateNameAndPwd(input){
	...
	<body>
		<div class="container_top">
			...
			你当前的位置：[系统管理 > 运维用户管理> 账户管理> 编辑账户]
		</div>
		<form method="post" action="Admin_saveYunweiUserInfo" id="infoform"
			name="infoform">
			<div class="content">
				<ul class="newobj">
					<li>
						...
						<tr>
							...
							<td>
								<input type="text" size="50" id="userid"
									value="<s:property value="yunweiUser.userid" />"
									name="yunweiUser.userid" maxlength="20" disabled="disabled" />
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td>
								<input type="text" size="50"
									value="<s:property value="yunweiUser.name" />"
									name="yunweiUser.name" maxlength="20" />
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td>
								<input type="text" size="50" id="pwd"
									value="<s:property value="yunweiUser.pwd" />"
									name="yunweiUser.pwd" maxlength="20" />
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td>
								<input id="cellphone" type="text" size="50"
									value="<s:property value="yunweiUser.cellphone" />"
									name="yunweiUser.cellphone" maxlength="20" />
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td>
								<input type="checkbox" onclick="changeStateOilele(this)"
									<s:if test="yunweiUser.oilele==1">checked="checked"</s:if>>
								</input>
								<input type="hidden" name="yunweiUser.oilele" id="oil"
									value="<s:if test="yunweiUser.oilele==1">1</s:if><s:else>0</s:else>" />
								锁机解锁权限
								<input type="checkbox" onclick="changeStateModify(this)"
									<s:if test="yunweiUser.modify==1">checked="checked"</s:if>>
								</input>
								<input type="hidden" name="yunweiUser.modify" id="mod"
									value="<s:if test="yunweiUser.modify==1">1</s:if><s:else>0</s:else>" />
								修改信息权限
								<input type="checkbox" onclick="changeStateExport(this)"
									<s:if test="yunweiUser.export==1">checked="checked"</s:if>>
								</input>
								<input type="hidden" name="yunweiUser.export" id="exp"
									value="<s:if test="yunweiUser.export==1">1</s:if><s:else>0</s:else>" />
								导出数据权限
							</td>
						</tr>
						<tr>
							...
							<td>
								<input type="text" disabled="disabled" size="50"
									value="<s:date name="yunweiUser.registertime" format="yyyy-MM-dd hh:mm" />"
									name="yunweiUser.registertime" />
							</td>
						</tr>
						<tr>
							<th scope="row">
				...
				<table border="0" class="btn_table">
					<tr>
						<td>
							...
							<a
								href="<s:url action="Admin_yunweiUserManager?pageIndex=1"></s:url>"
