...
function validateNameAndPwd(input){
	...
	<body>
		<div class="container_top">
			...
			你当前的位置：[系统管理 > 集团用户管理> 账户管理>编辑账户]
		</div>
		<form method="post" action="Admin_saveJituanUserInfo" id="infoform"
			name="infoform">
			<div class="content">
				<ul class="newobj">
					<li>
						...
						<tr>
							...
							<td>
								<input type="text" size="50" id="userid"
									value="<s:property value="jituanUser.userid" />"
									name="jituanUser.userid" maxlength="20" disabled="disabled" />
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td>
								<input type="text" size="50"
									value="<s:property value="jituanUser.name" />"
									name="jituanUser.name" maxlength="20" />
							</td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td>
								<input type="text" size="50" id="pwd"
									value="<s:property value="jituanUser.pwd" />"
									name="jituanUser.pwd" maxlength="20" />
							</td>
						</tr>
						<tr>
							<th scope="row">
						...
						<tr>
							...
							<td>
								<input type="checkbox" onclick="changeStateOilele(this)"
									<s:if test="jituanUser.oilele==1">checked="checked"</s:if>>
								</input>
								<input type="hidden" name="jituanUser.oilele" id="oil"
									value="<s:if test="jituanUser.oilele==1">1</s:if><s:else>0</s:else>" />
								锁机解锁权限
								<input type="checkbox" onclick="changeStateModify(this)"
									<s:if test="jituanUser.modify==1">checked="checked"</s:if>>
								</input>
								<input type="hidden" name="jituanUser.modify" id="mod"
									value="<s:if test="jituanUser.modify==1">1</s:if><s:else>0</s:else>" />
								修改信息权限
								<input type="checkbox" onclick="changeStateExport(this)"
									<s:if test="jituanUser.export==1">checked="checked"</s:if>>
								</input>
								<input type="hidden" name="jituanUser.export" id="exp"
									value="<s:if test="jituanUser.export==1">1</s:if><s:else>0</s:else>" />
								导出数据权限
							</td>
						</tr>
						<tr>
