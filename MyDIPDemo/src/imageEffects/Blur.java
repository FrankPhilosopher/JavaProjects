 ...
 * 
 * References: http://www.jasonwaltman.com/thesis/filter-blur.html http://www.blackpawn.com/texts/blur/default.html
 */
public class Blur {
	/**
	...
	public static ImageData blur(ImageData originalImageData, int radius) {
		/*
		 * This method will vertically blur all the pixels in a row at once. This blurring is performed incrementally to
		 * each row.
		 * 
		 * In order to vertically blur any given pixel, maximally (radius * 2 + 1) pixels must be examined. Since each
		 * of these pixels exists in the same column, they span across a series of consecutive rows. These rows are
		 * horizontally blurred before being cached and used as input for the vertical blur. Blurring a pixel
		 * horizontally and then vertically is equivalent to blurring the pixel with both its horizontal and vertical
		 * neighbours at once.
		 * 
		 * Pixels are blurred under the notion of a 'summing scope'. A certain scope of pixels in a column are summed
		 * then averaged to determine a target pixel's resulting RGB value. When the next lower target pixel is being
		 * calculated, the topmost pixel is removed from the summing scope (by subtracting its RGB) and a new pixel is
		 * added to the bottom of the scope (by adding its RGB). In this sense, the summing scope is moving downward.
		 */
		if (radius < 1)
			return originalImageData;
		// prepare new image data with 24-bit direct palette to hold blurred copy of image
		ImageData newImageData = new ImageData(originalImageData.width, originalImageData.height, 24, new PaletteData(
		...
		int cacheSize = radius * 2 + 1 > newImageData.height ? newImageData.height : radius * 2 + 1; // number of rows
																										// of imageData
																										// we cache
