...
import com.yj.smarthome.composite.SecurityControlComposite;
 ...
 * 
 */
public class SecurityControlEditor extends EditorPart implements IDeviceMap {

	public static final String ID = SecurityControlEditor.class.getName();
...
//	public LightControlEditor lce = new LightControlEditor();//灯控的编辑器
	...
	public void init(IEditorSite site, IEditorInput input) throws PartInitException {
		this.setSite(site);
		this.setInput(input);
		this.setPartName(SystemUtil.SECURITY_CONTROL);
...
//			deviceMap = XmlUtil.loadDevices(ProtocolUtil.DeviceType_Light);
			deviceMap = XmlUtil.loadDevices(SecurityControlXmlImpl.getInstance());
		} catch (Exception e) {
//			e.printStackTrace();
			//抛出异常！提示！比如文件不存在！
...
//			LightControlComposite composite = new LightControlComposite(container, SWT.NONE, (LightButton) entry.getValue());
			SecurityControlComposite composite = new SecurityControlComposite(container, SWT.NONE, (Security) entry.getValue());
