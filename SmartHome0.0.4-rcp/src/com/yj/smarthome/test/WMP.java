...
public class WMP extends Composite {
	...
	protected void createContents() {
		...
		try {
			...
			// µÚÈý¸ö²ÎÊýÊÇ×¢²á±íÖÐProgID¼üµÄÖµ£¬±ÈÈçWordµÄ¾ÍÊÇWord.Document.6              
			controlSite = new OleControlSite(frame, SWT.NO_TRIM, "WMPlayer.OCX.7");
//			controlSite = new OleControlSite(frame, SWT.NO_TRIM, "WMPlayer.OCX");  
			// IEµÄVersionIndependentProgIDÊÇShell.Explorer ÔÚ×¢²á±íÖÐÖ¸¶¨               
			// controlSite.setBackgroundImage(ResourceManager.getPluginImage(Activator.PLUGIN_ID, WMP.class,                
		...
		// ¸ù¾ÝÇ°ÃæµÄcontrolSite´´½¨oleautomation,ÕâÑù¾Í¿ÉÒÔµ÷ÓÃ¿Ø¼þµÄÊôÐÔºÍ·½·¨          
	...
	/**
	 * autoRewind Mode ¡ª¡ª¡ªindicating whether the tracks are rewound to the beginning after playing to the end.
	 * Default state is true.
	 * 
	 * loop Mode ¡ª¡ª¨C indicating whether the sequence of tracks repeats itself. Default state is false.
	 * 
	 * showFrame Mode ¡ª¡ª¡ª indicating whether the nearest video key frame is displayed at the current position when
	 * not playing. Default state is false. Has no effect on audio tracks.
	 * 
	 * shuffle Mode ¡ª¡ª¡ª- indicating whether the tracks are played in random order. Default state is false.
	 * 
	 * @param m
	 * @param flag
	 */
	...
	private OleAutomation getCurrentMedia() {
		OleAutomation o = null;
		int ids[] = player.getIDsOfNames(new String[] { "currentMedia" });
		if (ids != null) {
			o = player.getProperty(ids[0]).getAutomation();
		}
		return o;
	...
	public void play(String url) {
		...
		if (idsma != null) {
			...
			System.out.println("size=" + cmedia.length);
			if (cmedia != null) {
