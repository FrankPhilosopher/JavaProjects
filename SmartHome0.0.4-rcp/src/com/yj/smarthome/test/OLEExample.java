...
/**
 * OLEExample is an example that uses <code>org.eclipse.swt</code> libraries to implement a simple SWT window that can
 * host different Active X controls.
 * 
 * @since 3.3
 */
public class OLEExample {
	...
	void fileOpen() {
		...
		if (clientSite == null) {
			...
			if (index != -1) {
				String fileExtension = fileName.substring(index + 1);
				if (fileExtension.equalsIgnoreCase("wmv") || fileExtension.equalsIgnoreCase("mpg")
						|| fileExtension.equalsIgnoreCase("mpeg") || fileExtension.equalsIgnoreCase("avi")
	...
	public void open(Display display) {
		...
		// Button flashButton = new Button(buttons, SWT.PUSH);
		// flashButton.setText("New Word Document");
		// flashButton.addSelectionListener(new SelectionAdapter() {
		// public void widgetSelected(SelectionEvent e) {
		// if (((Button) e.widget).getSelection())
		// newClientSite("Word.Document");
		// }
